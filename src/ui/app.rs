...
use crate::ui::state::{AppState, UiModal};
use crate::ui::view;
...
fn apply_command(state: &mut AppState, interaction: &mut Interaction, cmd: UiCommand) -> bool {
    match cmd {
        ...
        UiCommand::PromptMultiline { title, hint, reply } => {
            state.modal = Some(UiModal::Multiline {
                title,
                hint,
                lines: Vec::new(),
                current_line: String::new(),
            });
            *interaction = Interaction::Multiline {
                reply,
                lines: Vec::new(),
                current_line: String::new(),
            };
            ...
            KeyCode::Enter => {
                if current_line.is_empty() {
                    if lines.is_empty() {
                        ...
                        let _ = reply.send(UiPromptResult::Input(lines.join("\n")));
                    }
                    *interaction = Interaction::None;
                    state.modal = None;
                } else {
                    ...
                    current_line.clear();
                    if let Some(UiModal::Multiline {
                        lines: modal_lines,
                        current_line: modal_current,
                        ..
                    }) = &mut state.modal
                    {
                        *modal_lines = lines.clone();
                        *modal_current = current_line.clone();
                    }
                }
            }
            KeyCode::Backspace => {
