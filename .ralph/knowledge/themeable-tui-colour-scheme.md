...
The TUI uses a `Theme` struct in `src/ui/theme.rs` that holds all color tokens. Two built-in themes exist: `dark` (default) and `light`.

## Resolution Order
1. `RALPH_THEME` env var (highest priority)
2. `[ui].theme` in `.ralph.toml`
...
See also: [[Ratatui UI Runtime]], [[Configuration Layers]], [[UI Event Routing and Plain Fallback]]
